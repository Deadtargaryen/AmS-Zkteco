...
export default validateRoute(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const transaction = await client.$transaction([deleteAttendance, deleteMember])
      res.status(200).json({ message: 'Member deleted successfully', transaction })
    } catch (e) {
      return prismaError(e, res)
    }
    ...
    try {
      ...
      const currentMember = await client.member.findUnique({
        where: {
      ...
      const member = await client.member.update({
        ...
        data: {
          ...
          dateOfBaptism: dateOfBaptism !== null && dateOfBaptism !== '' ? new Date(dateOfBaptism) : null,
          dateOfMembership: dateOfMembership !== null && dateOfBaptism !== '' ? new Date(dateOfMembership) : null,

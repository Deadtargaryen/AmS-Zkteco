...
export default validateRoute(async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const { page = 1, per_page = 10, year = currentYear, startMonth = 1, endMonth = 12, searchParams } = req.query
    ...
    // Modify the query to include the range of months
      ...
      select: {
        ...
        attendance: {
          where: {
            ...
            month: {
              gte: startMonth ? parseInt(startMonth as string) : undefined,
              lte: endMonth ? parseInt(endMonth as string) : undefined,
            },
          },
        },
      },

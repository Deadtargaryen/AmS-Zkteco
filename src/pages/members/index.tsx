...
import { useDeleteMemberMutation, useGetMembersQuery } from '../../../redux/store/api'
...
const Members = () => {
  const columns = [
    ...
    },
    {
      ...
      cell: (row: { id: any }) => (
        <Flex gap="1">
          <Tooltip fontSize="xs" hasArrow label="View Details" bg="gray.600" placement="top">
            <Link href={{ pathname: `/profile/${row.id}`, query: { isProfile: true } }}>
              <IconButton size="xs" variant="solid" colorScheme="green" aria-label="Search database" icon={<MdOpenInNew />} />
            </Link>
          </Tooltip>
  ...
  useEffect(() => {
    if (members) {
      setMaleMembers(members?.filter(member => member.gender === 'MALE').length)
      setFemaleMembers(members?.filter(member => member.gender === 'FEMALE').length)
      setTotalMembers(members?.length)
      setActiveMembers(members?.filter(member => member.status === 'ACTIVE').length)
